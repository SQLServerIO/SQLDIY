...
DROP TABLE dbo.ServerJobsHistory
DROP TABLE dbo.ServerJobSchedules
DROP TABLE dbo.ServerJobSteps
************************************************************************************************/
IF NOT EXISTS (SELECT *
               FROM   dbo.sysobjects
               WHERE  id = Object_id(N'[dbo].[ServerJobs]')
...
ELSE
BEGIN
    PRINT 'Table ServerJobs already exists'
END

IF NOT EXISTS (SELECT *
               FROM   dbo.sysobjects
               WHERE  id = Object_id(N'[dbo].[ServerJobSchedules]')
                      AND Objectproperty(id, N'IsUserTable') = 1)
BEGIN
...
ELSE
BEGIN
    PRINT 'Table ServerJobSchedules already exists'
END

IF NOT EXISTS (SELECT *
               FROM   dbo.sysobjects
               WHERE  id = Object_id(N'[dbo].[ServerJobSteps]')
                      AND Objectproperty(id, N'IsUserTable') = 1)
BEGIN
...
ELSE
BEGIN
    PRINT 'Table ServerJobSteps already exists'
END

IF NOT EXISTS (SELECT *
               FROM   dbo.sysobjects
               WHERE  id = Object_id(N'[dbo].[ServerJobHistory]')
                      AND Objectproperty(id, N'IsUserTable') = 1)
BEGIN
...
ELSE
BEGIN
    PRINT 'Table ServerJobHistory already exists'
END
--clear all tables before gathering new data
TRUNCATE TABLE dbo.ServerJobs
TRUNCATE TABLE dbo.ServerJobSchedules
TRUNCATE TABLE dbo.ServerJobSteps

insert into [dbo].[ServerJobs]
select 
...
--ServerJobSchedules First Stage
--pop [management].[dbo].[ServerJobSchedules]
declare @x int
declare @y int
declare @z int
...
--ServerJobSchedules second Stage

insert into [dbo].[ServerJobSchedules]
select 
	@@servername as ServerName,
	b.name JobName, 
...
insert into [dbo].[ServerJobSteps]
select 
	@@servername as ServerName
...
insert into [dbo].[ServerJobHistory]
select distinct
	InstanceId
	,ServerName
	,ServerJobName
